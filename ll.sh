...
show_help() {
  cat <<EOF
Usage: ll.sh [-e ENDPOINT] [aws-s3-ls-args...]

Runs: aws s3 ls with optional --endpoint-url.

Environment:
  AWS_ENDPOINT_URL  default endpoint if -e not provided

Examples:
  ll.sh s3://my-bucket/path/
  ll.sh -e http://localhost:4566 s3://my-bucket
EOF
}
...
exec aws --endpoint-url "$EP" s3 ls "$@"
